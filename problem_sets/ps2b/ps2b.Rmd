...
The maximum before-event-period is negative 22 years, coming from State 7
(Delaware), which implements the 'primary' law in 2003, the final year of the
dataset. The maximum after-event-period is positive at positive 19 years, coming
from State 32 (New York), which implements the 'primary' law in 1984, the fourth
year in dataset. Note, only 18 states passed 'primary' law during this period,
and states that did not pass such law will have zeroes for all dummies.
...
# Event Study Table
reg_1a_cov_cluster <- coeftest(reg_1a, vcov = vcovHC(reg_1a, type = "HC0", cluster = "state"))
reg_1b_cov_cluster <- coeftest(reg_1b, vcov = vcovHC(reg_1b, type = "HC0", cluster = "state"))
stargazer(reg_1a_cov_cluster, reg_1b_cov_cluster,
          title = "Event Study Regressions\\label{tab:event-study-dummy-trap2}",
...
# Event study table
reg_1c_cov_cluster <- coeftest(reg_1c, vcov = vcovHC(reg_1c, type = "HC0", cluster = "state"))
stargazer(reg_1c_cov_cluster,
...
Because the event study in part (b) is highly unbalanced in event time, we prefer
this max-min threshold specification of the event study analysis. However, the 
panel is still significantly unbalanced in even time even with the threshold
bins. Part (f) lets us explore an interacted specification that more consistently
estimates the treatment effects. Though we do not estimate corrected standard
errors for part (f), the estimated coefficients are qualititatively similar to
both parts (b) and (c).
...
We are somewhat more confident about the event study results presented above. As
you can see both graphically and in regression outputs for 1b, the coefficients
on event times before the implementation of primary seat belt law are not
significantly different from zero, passing the pre-trend tests, while the
coefficients on event times 2-3 years after the implementation and onward are
consistently statistically significant with a negative sign, more so with time.
Furthermore, event study design may allow us to look into dynamic effects, if we
were to believe that the policy chance have longer term compounding effects.
However, our exploration thus far reveals no insight as to why we have a general
negative trends, after partialling out the time and state fixed effects, which
starts even before the implementation of the law and continues years after the
implementation.
...
before the treated unit was treated (`r min(preperiod_years)` - `r max(preperiod_years)`).
...
From the distribution of MSPEs, we see that our treatment cannot be ruled out as
mere chance. It is not even more than one standard deviation from the mean MSPE
of placebo tests (one standard deviation is approximately 24).
...
In Figure \ref{fig:ratios}, we can see that the post/pre MSPE ratio of our true
treatment unit-synthetic control gap falls somewhat on the right side of the
distribution. We can calculate a p-value as the probability that the result we
got was not just from random chance. This is the number of MSPE ratios that are
equal to or greater than the Treatment Unit's MSPE ratio, divided by the total
number of ratios. The p-value for our actual synthetic control estimate is `r round(TU_rank/total_ratios, 3)`, so we cannot reject the null hypothesis that
the decrease in fatalities is merely from random variation in the data.
...
Our FE results from 3f suggest that primary seat belt laws do in fact decrease
traffic  fatalities (with strong statistical significance). This is close to
what our synthetic controls model would suggest if we were to only look at the
pre and post differences without any placebo tests for context. By performing
the placebo test, however, we find that the treatment effect appears to be just
a fluke. Many other control states exhibit a negative "effect" from the
arbitrary treatment and TU is not out of the ordinary. This leads us to doubt
the FE results we found in 3f. Perhaps we could run placebo FE for other states
