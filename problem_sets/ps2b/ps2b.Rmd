...
```{r Apply Synth1, cache=Cache, message=F}
...
```{r Create placebo gaps, cache=Cache, warning=F, message=F}
# Create list of control states
control_states = data %>%
    group_by(state) %>%
    mutate(control = ifelse(mean(primary) > 0, 0, 1)) %>%
...
```{r historgram of MSPE ratios}
MSPE = function(df, treatment_year) {
    pretreat = filter(df, year < treatment_year)
    posttreat = filter(df, year1 >= treatment_year)
    SSEpre = sum(pretreat$gaps)
}

gaps_df2 %>%
    group_by(unit) %>%
    # sum of pre-treatment squared gaps
    # sum of post-treatment squared gaps
    # ratio of post/pre MSE
    summarize()
```
