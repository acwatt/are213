...
```{R}
summary(twomiledata$hrs_82)

################ Make histogram

twomiledata_split <- twomiledata %>%
  mutate(hrs_82_1 = ifelse(hrs_82>28.5,  NA, hrs_82),
         hrs_82_0 = ifelse(hrs_82<28.5,  NA, hrs_82))

min_hrs <- min(twomiledata_split$hrs_82)
max_hrs <- max(twomiledata_split$hrs_82) + 1.5
ax1 <-seq(min_hrs, 28.5,by = 1.5)
ax0 <-seq(30, max_hrs, by = 1.5)
ax <- c(ax1, ax0)

hg1 <- hist(twomiledata_split$hrs_82_1, breaks = ax, plot = F)
hg0 <- hist(twomiledata_split$hrs_82_0, breaks = ax, plot = F)
