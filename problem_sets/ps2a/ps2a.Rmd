...
# Packages
```{r packages, results='hide', message=FALSE, warning = FALSE, echo = T}
library(haven)
library(foreign)
library(tinytex)
library(stargazer)
...
Including these covariates drastically changes our point estimate. When the covariates other than secondary are include, the effect essentially disappears though remains negative. However, once secondary is included, the effect remains insignificant but turns positive.
...
vcov_cluster <- (1104/1102)*(XX_inv) %*% (t(X_a) %*% v2_a %*% X_a) %*% XX_inv
(vcov_cluster[1,1]*(1104/1002))^(1/2) # 1.941604e-16
(vcov_cluster[2,2]*(1104/1002))^(1/2) # 2.443184e-15

# I think this is Robust SE. How can we cluster manually...
```
...
df_within <- aggregate(df_within, list(df$state), mean) %>%
                mutate(ln_fat_pc_agg = log(fatalities/population), ln_tvmt_pc_agg = log(totalvmt/population))

reg_3c_btw_nocov <- plm(ln_fat_pc_agg ~ primary, 
                            data = df_within, 
                            model = "between")
...
reg_3c_btw_cov <- plm(ln_fat_pc_agg ~ primary + secondary + college + beer + unemploy
                            + ln_tvmt_pc_agg + precip + snow32 + rural_speed + urban_speed,
                            data = df_within, 
                            model = "between")
```
