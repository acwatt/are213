...
```{r 3.b standard errors bivariate}
...
SE_b
```
```{r 3.b standard errors year FE}
library(lmtest)
library(sandwich)
coeftest(reg_a_yfe_lm, vcov = vcovHC(reg_a_yfe_lm, method="white1", type="HC0"))[[2]]
...
The between estimator is consistent in the random effects model (under the RE assumptions) but not under fixed effects. Thus, in order for this between estimator to be unbiased, individual state time-invarying effects, which now rests within the error term, cannot be correlated with the independent variables (includeding 'primary' and other time-varying covariates). We do not think this assumption holds here, given each states' time-invariant characteristics regarding average driving styles or fatality rate would likely impact state adoption of primary and secondary laws as well as driving conditions captured by time-varying covariates. With the between estimator, we am worried about SE, given we are down to 48 observations, one for each state, by averaging each state's observation over time.
...
df_diff <- df_diff %>%
              group_by(state) %>%
              mutate("{i}_diff" := eval(as.symbol(i)) - dplyr::lag(eval(as.symbol(i)), n= 2))
}
...
stargazer(reg3h_1, reg3h_2, reg3h_3, 
          title = "First, Second, and Long Difference Estimator",
          dep.var.caption = "Differences in Log(Fatality per Population)",
          dep.var.labels.include = FALSE, model.names = FALSE,
          column.labels = c("1st", "2nd", "Long"),
          add.lines=list(c('Time FE', 'Yes', 'Yes', 'No')),
          font.size = "footnotesize", column.sep.width = "1pt", no.space = TRUE, omit.stat=c("f", "ser"),
          type = "text", digits = 4)
```

We observe that 1st and 2nd difference estimator give very similar or slightly more negative coefficients estimates, while standard errors go down significantly. Perhaps, this may mean that effect of implementing primary law is more volatile but stabilizes over longer term period. Long term difference estimator has he most negative coefficient. Another key feature to notice is that for long difference estimator, we are taking the difference between the first and last year for each state, and we are left with one observation per state. Hence, thought not on primary variable, most of the standard errors significantly increase.
...
First differences provides the researcher with more observations than the 5-year logn differences and therefore potentially more power.
