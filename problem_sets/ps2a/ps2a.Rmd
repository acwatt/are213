...
```{r 3b Clustered SE (Manual Calculation)}
# Manually Calculate Clustered SE for Bivariate OLS
Y_a <- df[,"ln_fat_pc"] %>%
          as.matrix() #1104 x 1
...
The between estimator is consistent in the random effects model (under the RE assumptions) but not under fixed effects. Thus, in order for this between estimator to be unbiased, individual state time-invarying effects, which now rests within the error term, cannot be correlated with included 'primary' or other time-varying covariates. We do not think this assumption holds here, given each states' time-invariant characteristics regarding average driving styles or fatality rate would likely impact state adoption of primary and secondary laws as well as driving conditions captured by time-varying covariates. With the between estimator, we am worried about SE, given we are down to 48 observations, one for each state, by averaging each state's observation over time.
...
df_within <- df %>% 
                ...
                summarise(across(c(college, beer, primary, secondary, population, unemploy, 
                                   fatalities, totalvmt, precip, snow32, rural_speed, urban_speed, fat_pc, ln_fat_pc, ln_tvmt_pc), mean, na.rm = TRUE))
...
reg_3d_RE_tf <- plm(ln_fat_pc ~ primary,
                            data = df, 
                            model = "random",
                            index = c("state", "year"), effect = ("time"))

reg_3d_RE_covtf <- plm(ln_fat_pc ~ primary + secondary + college + beer + unemploy
                            + ln_tvmt_pc + precip + snow32 + rural_speed + urban_speed,
                            data = df, 
                            model = "random",
                            index = c("state", "year"), effect = ("time"))
...
stargazer(reg_3d_RE_bin, reg_3d_RE_tf, reg_3d_RE_covtf, reg_3d_pooled_bin, reg_3d_pooled_cov, 
          title = "Random Effects vs Pooled",
          dep.var.caption = "Log(Fatality per Population)",
          dep.var.labels.include = FALSE, model.names = FALSE,
          column.labels = c("RE", "RE", "RE", "Pooled", "Pooled"),
          keep = c("primary"),
          add.lines=list(c('Time FE', 'No','Yes', 'Yes', 'No', 'No'),
                         c('Covariates', 'No', 'No', 'Yes', 'No', 'Yes')),
          font.size = "footnotesize", column.sep.width = "1pt", no.space = TRUE, omit.stat=c("f", "ser"),
          type = "text", digits = 4)
```

Random Effects model will give unbiased estimates when the strict exogeneity condition and uncorrelated condition are satisfied. The advantages 
